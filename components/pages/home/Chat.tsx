...
const Chat = () => {
  ...
  return (
    <div
      className={cn(
        "absolute left-8 bottom-10  max-lg:hidden chat-container z-[55555]",
        isChatOpen && "h-[350px] w-[320px]"
      )}
    >
      {isChatOpen && (
        <div className="glass-effect lg:rounded-2xl w-full h-full lg:p-4 flex flex-col gap-2">
      ...
      {!isChatOpen && (
        <div
          className="glass-effect flex justify-center rounded-normal flex-row lg:flex-col items-center gap-1.5 lg:gap-2.5 max-lg:h-14 lg:w-14 p-2 max-lg:backdrop-blur-0 max-lg:border-none"
          onClick={handleChatOpen}
        >
          <IconButton className="size-10 lg:size-11 rounded-lg">
            <MessageCircleIcon className="size-4 lg:size-5" />
          </IconButton>
        </div>
      )}
    </div>
