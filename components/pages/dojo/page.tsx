...
const DojoPage = () => {
  ...
  const [changedPartsCount, setChangedPartsCount] = useState(0)
  ...
  useEffect(() => {
    ...
    // Calculate how many parts changed from default
    let changed = 0
    if (selectedTraits.hairId !== defaultTraits.hairId && selectedTraits.hairId !== 0) changed++
    if (selectedTraits.eyesId !== defaultTraits.eyesId && selectedTraits.eyesId !== 0) changed++
    if (selectedTraits.mouthId !== defaultTraits.mouthId && selectedTraits.mouthId !== 0) changed++
    if (selectedTraits.elementId !== defaultTraits.elementId && selectedTraits.elementId !== 0) changed++
    if (selectedTraits.bodyId !== defaultTraits.bodyId && selectedTraits.bodyId !== 0) changed++
    if (selectedTraits.marksId !== defaultTraits.marksId) changed++
    if (selectedTraits.powersId !== defaultTraits.powersId) changed++
    if (selectedTraits.accessoriesId !== defaultTraits.accessoriesId) changed++
    if (selectedTraits.backgroundId !== defaultTraits.backgroundId) changed++
    setChangedPartsCount(changed)
  }, [selectedTraits, traits, defaultTraits])
  ...
  const handleEquipParts = async () => {
    const totalCost = changedPartsCount * EQUIP_COST_PER_ITEM
    if (totalCost === 0) {
      toast.error("No changes made")
      return
    }
    if (profile?.stars && profile?.stars < totalCost) {
      toast.error(`You need ${totalCost} Stars to equip ${changedPartsCount} item(s) (${EQUIP_COST_PER_ITEM} Stars each)`)
      return
    }
              ...
              <div className="flex items-center gap-5">
                ...
                >
                  ...
                  <span className="text-2xs flex items-center gap-2 font-inter">
                    {changedPartsCount > 0
                      ? `${changedPartsCount} item${changedPartsCount > 1 ? "s" : ""} × ${EQUIP_COST_PER_ITEM}`
                      : `${EQUIP_COST_PER_ITEM}/item`}{" "}
